<template>
    <select class="form-control min-with-select" id="CboCampTipo" @change="onChange($event)" v-model="cboTipo" :required="required" >
...
<script>
    export default {
        ...
        methods:{
            ...
            emitEventCombo(data){
                EventBus.$emit('cboTipoSelect',data)
                EventBus.$emit('eventCboAlmacenSelect',data)
            },
            getDataTipo(){
